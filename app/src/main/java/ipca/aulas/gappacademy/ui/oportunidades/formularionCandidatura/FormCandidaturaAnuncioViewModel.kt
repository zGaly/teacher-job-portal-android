...
class FormularioCandidaturaAnuncioViewModel : ViewModel() {
    ...
    var anuncioId = mutableStateOf<String?>(null)
    ...
    suspend fun enviarCandidaturaAnuncio(): Boolean {
        ...
        return try {
            val result = repository.enviarCandidaturaAnuncio(
                name.value,
                email.value,
                phone.value,
                resumeUri.value,
            ...
            if (result) {
                ...
                errorMessage.value = "Erro ao enviar candidatura."
            }
            result
        } catch (e: Exception) {
